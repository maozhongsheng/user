...
import com.mk.ad.mapper.SysPermissionMapper;
import com.mk.ad.mapper.SysRoleMapper;
import com.mk.ad.service.*;
import com.mk.ad.utils.PageUtil;
import com.mk.ad.utils.TokenSettings;
...
//    }
@Override
    public Map detailInfo(String id) {
    Map result = new HashMap();
     SysRole sysRole = sysRoleMapper.selectByPrimaryKey(id);
     String name = sysRole.getName();
     String status = sysRole.getStatus().toString();
     String description = sysRole.getDescription();
    String role_power = sysRole.getRole_power();
    if(sysRole==null){
            log.error("传入 的 id:{}不合法",id);
            throw new BusinessException(BaseResponseCode.DATA_ERROR);
        }
    ...
    for (int i  = 0; i<permissionIdsByRoleId.size();i++){
          Map map = new HashMap<>();
            String s = permissionIdsByRoleId.get(i);
            SysPermission sysPermission = sysPermissionMapper.selectByPrimaryKey(s);
            String name1 = sysPermission.getName();
            map.put("id",s);
            map.put("name",name1);
            aa.add(map);
        ...
        result.put("permissions",aa);
        result.put("roleJurisdiction",role_power);

        return result;
    }

    @Override
    public List<SysRole> selectAllBy() {
        return sysRoleMapper.selectAllBy(new RolePageReqVO());
    }
    ...
    public Integer updateStatus(Map params) {
        Integer resutl =  sysRoleMapper.UpdateStatus(params);
